...
const (
	getAllReportsQuery    = "SELECT campaign.id, campaign.name as campaign_name, campaign.channel_account_id as channel_account_id, channel.name as channel_name, client.coster_id as client_id, client.coster_id as client_name, campaign.created_at, campaign.status FROM campaign JOIN channel ON campaign.channel_account_id = channel.name JOIN client ON campaign.client_id = client.coster_id"
	getSingleReportsQuery = `SELECT 
		campaign.id, 
		campaign.name as campaign_name, 
		channel.name as channel_name, 
		channel_account.id as channel_account_id, 
		channel_account.name as channel_account_name, 
		campaign.client_id, 
		...
		FROM campaign 
		JOIN channel_account 
		ON campaign.channel_account_id = channel_account.id 
		JOIN channel 
		ON channel_account.channel_id = channel.id 
	...
	campaign.id, 
		...
		campaign.created_at, 
		campaign_customer.customer_id 
	FROM campaign JOIN channel_account ON campaign.channel_account_id = channel_account.id JOIN channel ON channel_account.channel_id = channel.id JOIN client ON campaign.client_id = client.id JOIN campaign_summary ON campaign.id = campaign_summary.campaign_id JOIN campaign_customer ON campaign_summary.id = campaign_customer.summary_id WHERE campaign.deleted_at is null`
)
